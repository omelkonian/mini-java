...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        if (args.length != 3) {
            System.err.println("Usage: java Main <factsPath> <rulesPath/rule.iris> <queriesPath/queries.iris>");
            System.exit(-1);
        }            

        Parser parser = new Parser();

        final String factsPath = args[0];
        final String rulePath = args[1];
        final String queriesPath = args[2];
        ...
        File rulesFile = new File(rulePath);
